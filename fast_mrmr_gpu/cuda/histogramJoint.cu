...
inline __device__ void addByte(uint * s_histo, byte x, byte y, histoparams params) {
	uint pos = (x * params.valuesRangeY + y);

	if (pos >= (params.lap * MAX_BINS_PER_BLOCK) and pos < ((params.lap + 1) * MAX_BINS_PER_BLOCK)) {
...
__global__ void naiveHistoKernelJoint(histoparams params) {
	...
	i = threadIdx.x;
	while (i < MAX_BINS_PER_BLOCK) {
		atomicAdd(&(params.histo[params.lap*MAX_BINS_PER_BLOCK + i]), sharedHistogram[i]);
		i += blockDim.x;
	}
}

extern "C" void histogramNaiveJoint(t_data * d_vector1, t_data * d_vector2,
		histogram d_hist, unsigned int datasize, int blocks, int valuesRangeY, uint maxBins) {
	...
	params.valuesRangeY = valuesRangeY;
