...
inline __device__ void addWord(uchar *s_ThreadBase, uint data) {
	...
	addByte(s_ThreadBase, (data) & 0xFF);
	addByte(s_ThreadBase, (data >> 8) & 0xFF);
	addByte(s_ThreadBase, (data >> 16) & 0xFF);
	addByte(s_ThreadBase, (data >> 24) & 0xFF);
}
...
__global__ void histogram64Kernel(uint *d_PartialHistograms, data_t *d_Data,
	...
	const uint threadPos = ((threadIdx.x & ~(SHARED_MEMORY_BANKS * 4 - 1)) << 0)
			...
			| ((threadIdx.x & (SHARED_MEMORY_BANKS * 3)) >> LOG_SHARED_MEM);
...
#pragma unroll
	...
	if (threadIdx.x < HISTOGRAM64_BIN_COUNT) {
		uchar *s_HistBase = s_Hist
				+ UMUL(threadIdx.x, HISTOGRAM64_THREADBLOCK_SIZE);
...
#pragma unroll
		...
		for (uint i = 0; i < (HISTOGRAM64_THREADBLOCK_SIZE / 4); i++) {
			sum += s_HistBase[pos + 0] + s_HistBase[pos + 1]
					+ s_HistBase[pos + 2] + s_HistBase[pos + 3];
			pos = (pos + 4) & (HISTOGRAM64_THREADBLOCK_SIZE - 1);
		}

		d_PartialHistograms[blockIdx.x * HISTOGRAM64_BIN_COUNT + threadIdx.x] =
				sum;
	}
}
...
__global__ void mergeHistogram64Kernel(uint *d_Histogram,
		uint *d_PartialHistograms, uint histogramCount) {
	__shared__ uint data[MERGE_THREADBLOCK_SIZE];

	uint sum = 0;

	for (uint i = threadIdx.x; i < histogramCount; i +=
